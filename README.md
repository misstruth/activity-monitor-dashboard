...
在使用 Codex 进行 vibe coding，尤其是同时推进多个任务开发的时候，人很容易陷入一种反复切回 Codex 页面确认进度的状态。

你需要不断盯着：

- 当前线程到底有没有真正完成
- 这个任务是不是只是执行了一个小步骤
- 我刚刚改了哪些文件、跑了哪些命令、写了哪些提示词
- 现在机器资源是不是已经开始被某些进程拖慢

这种来回确认会打断思路，也会让人越来越乱、越来越焦虑。

所以才有了这个轻量级监控项目：把文件活动、终端命令、提示词记录、Codex 任务完成状态和系统资源集中到一个本地面板里，让你不用一直盯着 Codex 主界面，也能更安心地知道任务进展到了哪里。
...
![监控面板首页](docs/screenshots/dashboard-overview.png)

### 任务完成弹窗

下面这张图展示的是任务完成后的页面内 toast 提醒效果。

![任务完成弹窗](docs/screenshots/task-toast.png)
